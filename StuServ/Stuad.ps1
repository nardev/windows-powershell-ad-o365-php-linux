...
<#
  Provjera postoji li user                   
        .\stuADapi.ps1 -akcija p -user_id "test.p2"                

  Brisanje korisnika                   
        .\stuADapi.ps1 -akcija d -user_id "test.p2"

  Kreiranje korisnika  
  .\stuADapi.ps1 -akcija c -user_id "test.p2" -sifra userP2014 -ime Test -prezime Prezime3                
...
function NoviAduser ($ime, $prezime, $sifra ,$user_id)
{
   ...
   $upn = $user_id + "@students.local"
    #dodaj novog korisnika
    New-ADUser -Name ($ime  + " " +$prezime) -UserPrincipalName $upn -GivenName $ime -Surname $prezime -SamAccountName $user_id -DisplayName ($ime  + " " +$prezime) -AccountPassword (ConvertTo-SecureString -AsPlainText $sifra -Force) -Path "OU=SpaceAccounts,DC=students,DC=local"
